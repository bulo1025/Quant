...
for target in tqdm(target_list):
    ...
    data['low_1minbar'] = bar_data_re['low']      # 1分钟 close price 的 low bar
    data = data.dropna()
    # -----------------------------因子构建----------------------------#
    ...
    final_DF = final_DF.dropna()
    final_DF =final_DF.resample('1T').last().dropna()

#-----------------------------------期货和指数因子----------------------#
    # 生成1T的采样
    future_close = data_futures['Close'].resample('1T').last().dropna()
    index_close = data_index['Close'].resample('1T').last().dropna()
    ETF_close = data['Close'].resample('1T').last().dropna()
    # 计算基差一阶差分
    basis = (future_close - index_close).dropna()
    final_DF['basis_diff'] = basis.diff().dropna()
    # 计算期货-ETF价差 2H- 1day
    C = (future_close.rolling(20).mean() / ETF_close.rolling(20).mean()).dropna()
    final_DF['spread'] = (future_close - C * ETF_close).dropna()
    final_DF['midprice'] = final_DF['midprice'].diff().dropna()   # 将序列平稳化
    final_DF['target'] = (final_DF['midprice'].shift(3) - final_DF['midprice']).dropna()
